...
# Directories
BASEDIR="/data/adb/modules/Konyaku"
INT="/data/adb/.config/Konyaku"
RWD="$INT"
MSC="$BASEDIR"
SEARCH_PATHS="/vendor/lib64/egl /vendor/lib64/hw"
PROCESSED_FILE_LIST="$RWD/processed_files.txt"
PRELOAD_ENABLED="/data/adb/.config/Konyaku/APreload"
# Game Libs
...
monitorfiles() {
sendlog "I" "KonyakuAddons" "Connected to Encore"
notification "Encore Tweaks" "Konyaku addon successfully applied"
encoreapi="/data/adb/.config/encore/current_profile"
checkfile="/data/adb/modules/Konyaku/system/bin/sys.zirelia.inotifybin"
applyutil="/data/adb/modules/Konyaku/system/bin/sys.zirelia.utilityscript"
profile="/data/adb/modules/Konyaku/system/bin/sys.zirelia.profilesetting"
bypasstate="/data/adb/modules/Konyaku/system/bin/bypassCharge"
"$applyutil"
...
"$checkfile" -m -e modify "$encoreapi" | while read -r path action file; do
    ...
    case "$current_profile" in
        1) 
            "$profile"
            if [ "$(cat /data/adb/.config/Konyaku/mltweak)" = "1" ]; then
               mlpriority 1
            fi
            if [ "$(cat /data/adb/.config/Konyaku/showtoast)" = "1" ]; then
               toasttext "Applying Performance Profile..."
            fi
            if [ "$(cat /data/adb/.config/Konyaku/bypass_charge)" = "1" ]; then
               "$bypasstate" 
               sendlog "I" "KonyakuAddons" "Enabling Bypass Charge"
            fi
            ;;
        2)  
            "$profile"
            ...
            if [ "$(cat /data/adb/.config/Konyaku/bypass_charge)" = "1" ]; then
               "$bypasstate"
               sendlog "I" "KonyakuAddons" "Bypass Charge is disabled"
            fi
            ;;
        3)
            "$profile"
